...
class fetch_check():
    ...
    def __warning(self):
        ...
        if self.flag == 1:
            # 浊度：100-500mg/L
            if not 100 <= eval(self.zhuodu) <= 500:
                message += "浊度为" + self.zhuodu + "\n"
            # 溶氧：5-10mg/L
            if not 5 <= eval(self.rongyang) <= 10:
                message += "溶氧量为" + self.rongyang + "\n"
            # ph:5-8
            if not 5 <= eval(self.ph) <= 8:
                message += "PH为" + self.ph + "\n"
            # 温度：-1-25
            if not -1 <= eval(self.wehdu) <= 25:
                message += "温度为" + self.wehdu + "\n"
